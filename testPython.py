...
def loadillustrationcontent(illustrationname):
    big_dict = {}
    ref = dbs.reference('cartoons')
    snapshot = ref.child("illustrations").child(illustrationname).order_by_child("timestamp").limit_to_last(100).get()
    if snapshot:
        for value in snapshot.values():
            smalldict = {}
            smalldict['name'] = value['name']
...
def loadconthumourcontent(conthumourname):
    big_dict = {}
    ref = dbs.reference('cartoons')
    snapshot = ref.child("conthumour").child(conthumourname).order_by_child("timestamp").limit_to_last(100).get()
    if snapshot:
        for value in snapshot.values():
            smalldict = {}
            smalldict['name'] = value['name']
...
def loadvideos():
    ...
    snapshot = ref.child("combined").order_by_child("timestamp").limit_to_last(3).get()
    if snapshot:
        for value in snapshot.values():
            smalldict = {}
            smalldict['name'] = value['name']
...
def loadnewslocations():
    ...
    newsref = dbs.reference(f'news/news')
    newssnapshot = newsref.order_by_child("timestamp").limit_to_last(100000).get()
